...
3. Automatically clicks Delete button and types "Delete" in popup
4. Offers to reopen Cursor AI after account deletion
5. Cleans up Terminal and Chrome processes
"""
...
# Import automation script functions
def run_automation_script(profile_name: str = "carphucng2001a@gmail.com") -> bool:
    """Run the separate automation script."""
    try:
        import subprocess
        import sys
        ...
        automation_script = os.path.join(current_dir, "automate_cursor_deletion.py")
        
        if not os.path.exists(automation_script):
            print_colored("❌ Automation script not found", "red")
            return False
        
        print_colored("Running automation script...", "yellow")
        result = subprocess.run([
            sys.executable, automation_script, "--profile", profile_name
        ], capture_output=True, text=True)
        
        if result.returncode == 0:
            print_colored("✅ Automation script completed successfully!", "green")
            return True
        else:
            print_colored(f"❌ Automation script failed: {result.stderr}", "red")
            return False
            
    except Exception as e:
        print_colored(f"❌ Error running automation script: {e}", "red")
        return False
...
def ask_to_reopen_cursor(method: str = "auto", chrome_profile: str = "carphucng2001a@gmail.com") -> None:
    ...
    # Ask if user wants to automate account deletion
    print_colored("🤖 AUTOMATION OPTION", "cyan")
    print_colored("Would you like to automate the account deletion process?", "white")
    print_colored("This will automatically:", "white")
    print_colored("- Open a new Chrome window with your profile", "white")
    print_colored("- Navigate to Cursor settings page", "white")
    print_colored("- Click the Delete Account button", "white")
    print_colored("- Type 'Delete' in the confirmation field", "white")
    print_colored("- Click the final confirmation button", "white")
    print_colored("Note: This will open a new Chrome window, separate from the existing one", "yellow")
    print()
    
    while True:
        try:
            auto_response = input("Do you want to automate account deletion? (yes/no): ").strip().lower()
            if auto_response in ['yes', 'y', '1', 'true']:
                print_colored("Starting automated account deletion...", "yellow")
                
                success = run_automation_script(chrome_profile)
                if success:
                    print_colored("✅ Account deletion automation completed!", "green")
                else:
                    print_colored("⚠️ Automation had issues, please complete manually", "yellow")
                break
            elif auto_response in ['no', 'n', '0', 'false']:
                print_colored("✅ Manual account deletion mode selected.", "green")
                break
            else:
                print_colored("Please answer 'yes' or 'no'.", "yellow")
        except KeyboardInterrupt:
            print_colored("\n✅ Manual account deletion mode selected.", "green")
            break
...
def main():
    ...
    print_colored("5. Offer automated account deletion (if Selenium is available)", "white")
    print_colored("6. Ask if you want to reopen Cursor after account deletion", "white")
    print_colored("7. Clean up Terminal and Chrome after setup completion", "white")
    ...
    if processes_found or args.dry_run:
        ...
        if args.dry_run:
            ...
            if not args.dry_run:
                ask_to_reopen_cursor(args.cursor_method, args.chrome_profile)
    else:
