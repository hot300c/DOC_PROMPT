...
// public class ws_QuanLyTapTrung_NEW_BK(
...
//     private class BilInvoiceDetailDto
//     {
...
//         public int? CreatedDateAsInt { get; set; }
//         public int? ServiceId { get; set; }
//         public bool? IsRefund { get; set; }
//         public string? NoiDung { get; set; }
//         public long? NgayHopDongAsInt { get; set; }
...
//         // 235959 là giờ phút giây lớn nhất trong ngày (23:59:59), dùng để lấy hết dữ liệu trong ngày đến cuối ngày
//         var denNgayAsBigInt = denNgayAsInt * 1_000_000L + 235959;
//         var checksumFacId = DataUtils.Checksum(@params.FacId);

//         return HandleFull(@params, tuNgayAsInt, denNgayAsInt, tuNgayAsBigInt, denNgayAsBigInt, checksumFacId);
...
//     // GetFacAdmissionsWithTuoi: Lấy danh sách các lần nhập viện (FacAdmissions) trong khoảng thời gian chỉ định,
//     // đồng thời bổ sung thông tin tuổi (TuoiAsInt) của bệnh nhân tại thời điểm nhập viện.
//     // Hàm này thực hiện join giữa bảng nhập viện và bảng bệnh nhân để tính tuổi dựa trên ngày sinh và ngày nhập viện.
...
//         var bilInvoiceDetails = GetBilInvoiceDetails(@params, tuNgayAsBigInt, denNgayAsBigInt, checksumFacId);
//         var khachDatTruocs = bilInvoiceDetails.Where(t => t.Loai == "Đặt trước").Select(t => t.PatientId).ToHashSet();
...
//         long denNgayAsBigInt,
//         int checksumFacId,
//         List<BilInvoiceDetailDto> bilInvoiceDetails
//     )
//     {
//         var result = new Dictionary<string, object>();
...
//             select new
//             {
//                 PatientPay = Math.Round((decimal)g.Sum(g1 => g1.PatientPay ?? 0), 0),
//                 TongTienGiam = g.Key.TongTienGiam > 0
//                     ? g.Key.TongTienGiam ?? 0
//                     : Math.Round((decimal)g.Sum(g1 => g1.SoTienGiam ?? 0), 0),
//             }
//         ).ToList();
...
//         ).ToList();
//         decimal tongBilDatTruoc = bilDatTruocs.Sum(b => b.PatientPay ?? 0) - bilDatTruocs.Sum(b => b.SoTienGiam ?? 0);
...
//         var bilInvoiceDetails = GetBilInvoiceDetails(
//             @params,
//             tuNgayAsInt * 1_000_000L,
//             denNgayAsInt * 1_000_000L + 235959,
//             DataUtils.Checksum(@params.FacId)
//         );
...
//         var tongSoKhachBuoiSang = facAdmissions.Where(a => a.IsChieu == false).Count();
//         result["TongSoKhachBuoiSang"] = tongSoKhachBuoiSang;

//         // Tính toán khách BSGT
//         var khachBsgt = clinicalSessions.Where(cs => cs.ServiceId == 1).Count();
...
//     private List<BilInvoiceDetailDto> GetBilInvoiceDetails(
//         Parameters @params,
//         long tuNgayAsBigInt,
//         long denNgayAsBigInt,
//         int checksumFacId
...
//             .Where(bd => bd.FacId == @params.FacId && bd.CreatedDateAsInt.Between(tuNgayAsBigInt, denNgayAsBigInt))
//             .Select(bd => new BilInvoiceDetailDto
//             {
//                 InvoiceId = bd.InvoiceId,
//                 PatientId = bd.PatientId,
//                 Reason = bd.Reason,
//                 PatientPay = bd.PatientPay,
//                 SoTienGiam = bd.SoTienGiam,
//                 RealTotal = bd.RealTotal,
//                 Loai = bd.Loai,
//                 FacId = bd.FacId,
//                 CreatedDateAsInt = bd.CreatedDateAsInt,
//                 ServiceId = bd.ServiceId,
//                 IsRefund = bd.IsRefund,
//                 NoiDung = bd.NoiDung,
//                 NgayHopDongAsInt = bd.NgayHopDongAsInt,
//                 RefundType = bd.RefundType,
//                 TongTienGiam = bd.TongTienGiam,
//             })
//             .ToList();
//     }
...
//             .Where(hd =>
//                 hd.FacId == @params.FacId && hd.CreatedDateAsInt.Between(tuNgayAsInt, denNgayAsInt) && hd.IsPaid == true
//             )
...
//         var tongTiepNhan =
//             detailedStats.GetValueOrDefault("TiepNhanTheoNguon", new Dictionary<int, int>()) as Dictionary<int, int>;
//         if (tongTiepNhan != null)
//         {
//             int tongSoTiepNhan = tongTiepNhan.Values.Sum();
//             table.Rows.Add("Tổng số tiếp nhận", tongSoTiepNhan);
//         }
...
//     // Handler methods for different report types
//     private DataSet HandleChiTietTiepNhan(Parameters @params, int tuNgayAsInt, int denNgayAsInt, int checksumFacId)
//     {
//         var dataSet = new DataSet();

//         // Get detailed admission data
//         var facAdmissions = GetFacAdmissionsWithTuoi(@params, tuNgayAsInt, denNgayAsInt);

//         // Create detailed admission table
//         var table = new DataTable("ChiTietTiepNhan");
//         table.Columns.Add("FacAdmissionId", typeof(Guid));
//         table.Columns.Add("PatientId", typeof(Guid));
//         table.Columns.Add("FacId", typeof(string));
//         table.Columns.Add("AdmitDate", typeof(DateTime));
//         table.Columns.Add("NguonTiepNhan", typeof(int));
//         table.Columns.Add("Tuoi", typeof(int));
//         table.Columns.Add("IsChieu", typeof(bool));

//         foreach (var admission in facAdmissions)
//         {
//             table.Rows.Add(
//                 admission.FacAdmissionId,
//                 admission.PatientId ?? Guid.Empty,
//                 admission.FacId,
//                 admission.AdmitDate,
//                 admission.NguonTiepNhan,
//                 admission.TuoiAsInt,
//                 admission.IsChieu
//             );
//         }
...
//     private DataSet HandleChiTietTiepNhanDiaLy(Parameters @params, int tuNgayAsInt, int denNgayAsInt, int checksumFacId)
//     {
//         var dataSet = new DataSet();

//         // Get admission data with geographic information
//         var facAdmissions = GetFacAdmissionsWithTuoi(@params, tuNgayAsInt, denNgayAsInt);

//         // Create geographic analysis table
//         var table = new DataTable("ChiTietTiepNhanDiaLy");
//         table.Columns.Add("TinhThanh", typeof(string));
//         table.Columns.Add("SoLuong", typeof(int));

//         // Group by geographic location (simplified - you may need to add geographic fields to your DTOs)
//         var geographicStats = facAdmissions
//             .GroupBy(a => "Chưa phân loại") // Placeholder - implement actual geographic logic
//             .Select(g => new { TinhThanh = g.Key, SoLuong = g.Count() });

//         foreach (var stat in geographicStats)
//         {
//             table.Rows.Add(stat.TinhThanh, stat.SoLuong);
//         }
...
//     private DataSet HandleChiTietDoanhThu(
//         Parameters @params,
//         long tuNgayAsBigInt,
//         long denNgayAsBigInt,
//         int checksumFacId
...
//         // Create detailed revenue table
//         var table = new DataTable("ChiTietDoanhThu");
//         table.Columns.Add("InvoiceId", typeof(Guid));
//         table.Columns.Add("PatientId", typeof(Guid));
//         table.Columns.Add("Reason", typeof(string));
//         table.Columns.Add("PatientPay", typeof(decimal));
//         table.Columns.Add("SoTienGiam", typeof(decimal));
//         table.Columns.Add("RealTotal", typeof(decimal));
//         table.Columns.Add("Loai", typeof(string));

//         foreach (var invoice in bilInvoiceDetails)
//         {
//             table.Rows.Add(
//                 invoice.InvoiceId,
//                 invoice.PatientId,
...
//     private DataSet HandleChiTietKhamBenh(Parameters @params, int tuNgayAsInt, int denNgayAsInt, int checksumFacId)
//     {
//         var dataSet = new DataSet();

//         // Get physician admission data
//         var physicianAdmissions = GetPhysicianAdmissions(@params, tuNgayAsInt, denNgayAsInt);

//         // Create detailed examination table
//         var table = new DataTable("ChiTietKhamBenh");
//         table.Columns.Add("PhysicianAdmissionId", typeof(Guid));
//         table.Columns.Add("PatientId", typeof(Guid));
//         table.Columns.Add("RoomId", typeof(int));
//         table.Columns.Add("AdmitDate", typeof(DateTime));
//         table.Columns.Add("TgBatDauKham", typeof(DateTime));
//         table.Columns.Add("IsKhongDuocTiem", typeof(bool));
//         table.Columns.Add("IsPracticed", typeof(bool));

//         foreach (var admission in physicianAdmissions)
//         {
//             table.Rows.Add(
//                 admission.PhysicianAdmissionId,
//                 admission.PatientId,
//                 admission.RoomId,
//                 admission.AdmitDate,
//                 admission.TgBatDauKham,
//                 admission.IsKhongDuocTiem,
//                 admission.IsPracticed
//             );
//         }
...
//     private DataSet HandleChiTietPhongTiem(Parameters @params, int tuNgayAsInt, int denNgayAsInt, int checksumFacId)
//     {
//         var dataSet = new DataSet();

//         // Get clinical sessions for vaccination
//         var clinicalSessions = GetClinicalSessions(@params, tuNgayAsInt, denNgayAsInt);
//         var clinicalSessionsVaccine = GetClinicalSessionsVaccine(@params, tuNgayAsInt, denNgayAsInt, checksumFacId);

//         // Create vaccination room table
//         var table = new DataTable("ChiTietPhongTiem");
//         table.Columns.Add("ClinicalSessionId", typeof(Guid));
//         table.Columns.Add("PatientId", typeof(Guid));
//         table.Columns.Add("RoomId", typeof(int));
//         table.Columns.Add("ServiceId", typeof(int));
//         table.Columns.Add("ProductTypeId", typeof(int));
//         table.Columns.Add("IsDuocTiem", typeof(bool));
//         table.Columns.Add("CompletedOn", typeof(DateTime));

//         foreach (var session in clinicalSessionsVaccine)
//         {
//             table.Rows.Add(
//                 session.ClinicalSessionId,
//                 session.PatientId,
//                 session.RoomId,
//                 session.ServiceId,
...
//     private DataSet HandleChiTietThoiGian(Parameters @params, int tuNgayAsInt, int denNgayAsInt, int checksumFacId)
//     {
//         var dataSet = new DataSet();
...
//         var table = new DataTable("ChiTietThoiGian");
//         table.Columns.Add("Gio", typeof(int));
//         table.Columns.Add("SoLuong", typeof(int));

//         // Group by hour
//         var hourlyStats = facAdmissions
//             .GroupBy(a => a.AdmitOn?.Hour ?? 0)
//             .OrderBy(g => g.Key)
//             .Select(g => new { Gio = g.Key, SoLuong = g.Count() });

//         foreach (var stat in hourlyStats)
//         {
//             table.Rows.Add(stat.Gio, stat.SoLuong);
//         }
...
//     private DataSet HandleChiTietDatTruoc(Parameters @params, int tuNgayAsInt, int denNgayAsInt, int checksumFacId)
//     {
//         var dataSet = new DataSet();
...
//         var datTruocs = bilInvoiceDetails.Where(t => t.Loai == "Đặt trước").ToList();

//         // Create advance booking table
//         var table = new DataTable("ChiTietDatTruoc");
//         table.Columns.Add("InvoiceId", typeof(Guid));
//         table.Columns.Add("PatientId", typeof(Guid));
//         table.Columns.Add("PatientPay", typeof(decimal));
//         table.Columns.Add("SoTienGiam", typeof(decimal));
...
//         foreach (var datTruoc in datTruocs)
//         {
//             table.Rows.Add(
//                 datTruoc.InvoiceId,
//                 datTruoc.PatientId,
...
//     private DataSet HandleChiTietKhongDuocTiem(Parameters @params, int tuNgayAsInt, int denNgayAsInt, int checksumFacId)
//     {
//         var dataSet = new DataSet();

//         // Get patients who cannot be vaccinated
//         var physicianAdmissions = GetPhysicianAdmissions(@params, tuNgayAsInt, denNgayAsInt);
//         var khongDuocTiem = physicianAdmissions.Where(a => a.IsKhongDuocTiem == true).ToList();

//         // Create cannot vaccinate table
//         var table = new DataTable("ChiTietKhongDuocTiem");
//         table.Columns.Add("PhysicianAdmissionId", typeof(Guid));
//         table.Columns.Add("PatientId", typeof(Guid));
//         table.Columns.Add("RoomId", typeof(int));
//         table.Columns.Add("AdmitDate", typeof(DateTime));
//         table.Columns.Add("DischargedOn", typeof(DateTime));
//         table.Columns.Add("PrimaryDoctor", typeof(Guid));

//         foreach (var admission in khongDuocTiem)
//         {
//             table.Rows.Add(
//                 admission.PhysicianAdmissionId,
//                 admission.PatientId,
...
//     private DataSet HandleChiTietXuatKhoVaccine(
//         Parameters @params,
//         int tuNgayAsInt,
//         int denNgayAsInt,
//         int checksumFacId
...
//         // Get vaccine inventory data
//         var invApprovedOuts = db
//             .QAHosGenericDB.InvApprovedOuts.With(SqlServerHints.Table.NoLock)
//             .Where(iao => iao.FacId == @params.FacId && iao.CreatedDateAsInt.Between(tuNgayAsInt, denNgayAsInt))
//             .ToList();

//         // Create vaccine inventory table
//         var table = new DataTable("ChiTietXuatKhoVaccine");
//         table.Columns.Add("ApprovedOutId", typeof(Guid));
//         table.Columns.Add("ApprovedOutNo", typeof(string));
//         table.Columns.Add("RequestStockId", typeof(int));
//         table.Columns.Add("CreatedOn", typeof(DateTime));
//         table.Columns.Add("FacId", typeof(string));

//         foreach (var approvedOut in invApprovedOuts)
//         {
//             table.Rows.Add(
//                 approvedOut.ApprovedOutId,
//                 approvedOut.ApprovedOutNo,
//                 approvedOut.RequestStockId,
//                 approvedOut.CreatedOn,
//                 approvedOut.FacId
//             );
//         }
...
//     private DataSet HandleBenhNhanNhieuCongKham(
//         Parameters @params,
//         int tuNgayAsInt,
//         int denNgayAsInt,
//         int checksumFacId
...
//         // Find patients with multiple admissions on the same day
//         var benhNhanNhieuCongKham = facAdmissions
//             .GroupBy(a => new { a.PatientId, a.AdmitDateAsInt })
//             .Where(g => g.Count() > 1)
//             .Select(g => new
//             {
//                 PatientId = g.Key.PatientId,
//                 AdmitDate = g.Key.AdmitDateAsInt,
//                 SoCongKham = g.Count(),
//             })
//             .ToList();

//         // Create multiple admissions table
//         var table = new DataTable("BenhNhanNhieuCongKham");
//         table.Columns.Add("PatientId", typeof(Guid));
//         table.Columns.Add("AdmitDate", typeof(int));
//         table.Columns.Add("SoCongKham", typeof(int));

//         foreach (var benhNhan in benhNhanNhieuCongKham)
//         {
//             table.Rows.Add(benhNhan.PatientId, benhNhan.AdmitDate, benhNhan.SoCongKham);
//         }
